...
is($config->_get_filename('common_config_test'), $test_conf, '_get_filename finds ./common_config_test.conf');
stderr_like 
	{ is($config->_get_filename('does_not_exist'), undef, '_get_filename does not find non existing file does_not_exist.conf'); } 
	qr/does_not_exist\.conf/, '_get_filename prints a message because it could not find the file';
...
unlink("$SELF_DIR/common_config_test.conf");
